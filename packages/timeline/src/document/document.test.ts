...
describe('getDuration', () => {
  ...
  it('is the far edge of the latest clip across all tracks', () => {
    ...
    doc = unwrap(addClip(doc, { trackId: VIDEO, assetId: ASSET_A, start: ms(0), duration: ms(5000) }));
    doc = unwrap(addClip(doc, { trackId: AUDIO, assetId: ASSET_B, start: ms(2000), duration: ms(10_000) }));
...
describe('addClip', () => {
  ...
  it('defaults sourceStart to zero when omitted', () => {
    const doc = unwrap(
      addClip(createEmptyTimeline(), { trackId: VIDEO, assetId: ASSET_A, start: ms(0), duration: ms(1000) }),
    );
  ...
  it('rejects a clip that overlaps an existing one on the same track', () => {
    ...
    const result = addClip(doc, { trackId: VIDEO, assetId: ASSET_B, start: ms(2000), duration: ms(1000) });
    expect(result.ok).toBe(false);
  ...
  it('accepts a clip that starts exactly where another ends', () => {
    ...
    const result = addClip(doc, { trackId: VIDEO, assetId: ASSET_B, start: ms(5000), duration: ms(1000) });
    expect(result.ok).toBe(true);
  });

  it('keeps a track sorted by start regardless of insertion order', () => {
    let doc = createEmptyTimeline();
    doc = unwrap(addClip(doc, { trackId: VIDEO, assetId: ASSET_A, start: ms(5000), duration: ms(1000) }));
    doc = unwrap(addClip(doc, { trackId: VIDEO, assetId: ASSET_B, start: ms(0), duration: ms(1000) }));
...
describe('findFreeStart', () => {
  ...
  it('cascades past back-to-back clips', () => {
    let doc = createEmptyTimeline();
    doc = unwrap(addClip(doc, { trackId: VIDEO, assetId: ASSET_A, start: ms(0), duration: ms(5000) }));
    doc = unwrap(addClip(doc, { trackId: VIDEO, assetId: ASSET_B, start: ms(5000), duration: ms(5000) }));
  ...
  it('skips a gap too small for the clip', () => {
    let doc = createEmptyTimeline();
    doc = unwrap(addClip(doc, { trackId: VIDEO, assetId: ASSET_A, start: ms(0), duration: ms(1000) }));
    doc = unwrap(addClip(doc, { trackId: VIDEO, assetId: ASSET_B, start: ms(2000), duration: ms(1000) }));
  ...
  it('uses a gap that exactly fits the clip', () => {
    let doc = createEmptyTimeline();
    doc = unwrap(addClip(doc, { trackId: VIDEO, assetId: ASSET_A, start: ms(0), duration: ms(1000) }));
    doc = unwrap(addClip(doc, { trackId: VIDEO, assetId: ASSET_B, start: ms(6000), duration: ms(1000) }));
...
describe('moveClip', () => {
  ...
  it('rejects a move that would overlap another clip', () => {
    let doc = createEmptyTimeline();
    doc = unwrap(addClip(doc, { trackId: VIDEO, assetId: ASSET_A, start: ms(0), duration: ms(1000) }));
    doc = unwrap(addClip(doc, { trackId: VIDEO, assetId: ASSET_B, start: ms(5000), duration: ms(1000) }));
    const clipId = doc.tracks[0]!.clips[0]!.id;
...
describe('trimClip', () => {
  it('trims the start edge, shifting sourceStart forward by the same amount', () => {
    let doc = createEmptyTimeline();
    doc = unwrap(
      addClip(doc, { trackId: VIDEO, assetId: ASSET_A, start: ms(1000), duration: ms(5000), sourceStart: ms(200) }),
    );
    const clipId = doc.tracks[0]!.clips[0]!.id;
  ...
  it('trims the end edge, shortening duration without touching sourceStart', () => {
    let doc = createEmptyTimeline();
    doc = unwrap(
      addClip(doc, { trackId: VIDEO, assetId: ASSET_A, start: ms(0), duration: ms(5000), sourceStart: ms(200) }),
    );
    const clipId = doc.tracks[0]!.clips[0]!.id;
  ...
  it('rejects trimming the start before the available source', () => {
    let doc = createEmptyTimeline();
    doc = unwrap(
      addClip(doc, { trackId: VIDEO, assetId: ASSET_A, start: ms(1000), duration: ms(1000), sourceStart: ms(0) }),
    );
    const clipId = doc.tracks[0]!.clips[0]!.id;
...
describe('splitClip', () => {
  it('splits one clip into two contiguous clips sharing the source asset', () => {
    let doc = createEmptyTimeline();
    doc = unwrap(
      addClip(doc, { trackId: VIDEO, assetId: ASSET_A, start: ms(0), duration: ms(10_000), sourceStart: ms(0) }),
    );
    const clipId = doc.tracks[0]!.clips[0]!.id;
    ...
    expect(clips[1]).toMatchObject({ start: 4000, duration: 6000, sourceStart: 4000, assetId: ASSET_A });
  });

  it('gives the two halves different ids', () => {
    let doc = createEmptyTimeline();
    doc = unwrap(addClip(doc, { trackId: VIDEO, assetId: ASSET_A, start: ms(0), duration: ms(10_000) }));
    const clipId = doc.tracks[0]!.clips[0]!.id;
